...
interface TeamMemberCardProps extends TeamMemberType{
...
function TeamMemberField({href, children}:TeamMemberFieldProps){
	return <a href={href} className="underline flex flex-row gap-2 items-center">{children}</a>
}

export default function TeamMemberCard({ name, title, website, motto, email, profilePicture }: TeamMemberCardProps) {
	return (
		<>
					...
					<Card className="h-full">
						<CardHeader>
							...
							<CardDescription className="flex flex-col items-center justify-center mt-2 gap-1" >
								{motto && <q className="text-md pb-2 text-center text-md">{motto}</q>}
								{email && <TeamMemberField href={"mailto:"+email}><AtSignIcon size={16}/>{email}</TeamMemberField>}
								{website && <TeamMemberField href={website}><Link size={16}/>{website.replace(/(^\w+:|^)\/\//, '')}</TeamMemberField>}
							</CardDescription>
						</CardHeader>
					</Card>
				</div>
