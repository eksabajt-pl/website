...
const arrays = (stars:number) => {
    return Array(stars).fill(<FontAwesomeIcon className="text-lg ml-[10px] text-green-500" icon={faStar} />)
...
{jsonFile?.map((value,index)=> {
    return(
        <div className="h-[100%] p-4 my-3 rounded-2xl w-[33%] max-md:w-[100%] max-md:min-h[150px]  max-lg:p-1" key={index}><div className='flex flex-row justify-start'><div className="flex justify-start items-center  text-3xl my-3 text-stone-850 font-black max-md:text-2xl max-md:m-0">{value.name}</div><div className=' flex justify-center items-center my-3  text-xl font-bold text-center'>{arrays(value.stars)}</div></div><div className="p-1 text-xl leading-10 max-md:leading-7 text-wrap max-md:text-base">{value.content}</div></div>
    );
})}
    </div>
</div>
